...
public class RandomPackage extends JavaPlugin {
    ...
    public static FileConfiguration getConfig(RPConfigs rpc) {
        if (rpc.equals(RPConfigs.COMMANDS_FEATURES)) {
            return commandsFeatures;
        } else if (rpc.equals(RPConfigs.DROP_PACKAGES)) {
            return dropPackages;
        } else if (rpc.equals(RPConfigs.ENCHANTMENTS)) {
            return enchantments;
        } else if (rpc.equals(RPConfigs.ENCHANTMENTS_INFO)) {
            return enchantmentsInfo;
        } else if (rpc.equals(RPConfigs.MASTER)) {
            return masterconfig;
        } else if (rpc.equals(RPConfigs.MESSAGES)) {
            return messages;
        } else if (rpc.equals(RPConfigs.SOUNDS)) {
            return sounds;
        } else if (rpc.equals(RPConfigs.SHOP)) {
            return shop;
        } else if (rpc.equals(RPConfigs.OTHER_GUIS)) {
            return otherGuis;
        ...
        } else if (rpc.equals(RPConfigs.GKITS)) {
            return gkits;
        } else if (rpc.equals(RPConfigs.VKITS)) {
            return vkits;
        } else {
            return null;
        }
    ...
    public void onEnable() {
        ...
        for (int i = 1; i <= 76; i++) {
            if (i == 1) {
                ...
                file = new File(RandomPackage.getPlugin.getDataFolder() + File.separator + "drop packages" + File.separator, "--god.yml");
            } else if (i == 72) {
                file = new File(RandomPackage.getPlugin.getDataFolder() + File.separator + "drop packages" + File.separator, "--legendary.yml");
            } else if (i == 73) {
                file = new File(RandomPackage.getPlugin.getDataFolder() + File.separator + "drop packages" + File.separator, "--ultimate.yml");
            } else if (i == 74) {
                file = new File(RandomPackage.getPlugin.getDataFolder() + File.separator + "drop packages" + File.separator, "-elite.yml");
            } else if (i == 75) {
                file = new File(RandomPackage.getPlugin.getDataFolder() + File.separator + "drop packages" + File.separator, "-unique.yml");
            } else if (i == 76) {
                file = new File(RandomPackage.getPlugin.getDataFolder() + File.separator + "drop packages" + File.separator, "simple.yml");
                //
            } else {
                file = null;
            }
            if (!(file == null)) {
                ...
                //
                if (i == 1) {
                    commandsFeatures = fileConfig;
                } else if (i == 2) {
                    masterconfig = fileConfig;
                } else if (i == 3) {
                    messages = fileConfig;
                } else if (i == 4) {
                    enchantments = fileConfig;
                } else if (i == 5) {
                    enchantmentsInfo = fileConfig;
                } else if (i == 8) {
                    otherGuis = fileConfig;
                } else if (i == 9) {
                    shop = fileConfig;
                } else if (i == 10) {
                    gkits = fileConfig;
                    ...
                    dropPackages = fileConfig;
                }
            }
        }
        //

        RPListeners rp = new RPListeners();
        pm.registerEvents(rp, this);
        pm.registerEvents(new RandomPackageAPI(), this);

        getCommand("givedp").setExecutor(rp);
        getCommand("gkit").setExecutor(rp);
        getCommand("randompackage").setExecutor(rp);
        getCommand("disabledenchants").setExecutor(rp);
        getCommand("vkit").setExecutor(rp);
        //

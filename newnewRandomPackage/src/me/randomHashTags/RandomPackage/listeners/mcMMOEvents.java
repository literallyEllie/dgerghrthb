...
public class mcMMOEvents {
        ...
        if (!(event.getXpGainReason().equals(XPGainReason.UNKNOWN))) {
            ...
            for (String enchant : api.getEnchantmentsOnItem(item)) {
                ...
                if (random.nextInt(100) <= chance) {
                    if (OEN.equals("Nimble") && event.getSkill().equals(SkillType.ACROBATICS)) {
                        ExperienceAPI.addRawXP(event.getPlayer(), SkillType.ACROBATICS.name(), (int) event.getRawXpGained() * level);
                        ...
                        if (item.getType().name().endsWith("SWORD")) {
                            ExperienceAPI.addRawXP(event.getPlayer(), SkillType.SWORDS.name(), event.getXpGained() * level);
                        } else if (item.getType().name().endsWith("_AXE")) {
                            ExperienceAPI.addRawXP(event.getPlayer(), SkillType.AXES.name(), event.getXpGained() * level);
                        }
                    } else if (OEN.equals("Skilling") && event.getSkill().equals(SkillType.GATHERING_SKILLS)) {
                        if (item.getType().name().endsWith("SPADE")) {
                            ExperienceAPI.addRawXP(event.getPlayer(), SkillType.EXCAVATION.name(), event.getXpGained() * level);
                        } else if (item.getType().name().endsWith("PICKAXE")) {
                            ExperienceAPI.addRawXP(event.getPlayer(), SkillType.MINING.name(), event.getXpGained() * level);
                        } else if (item.getType().name().endsWith("_AXE")) {
                            ExperienceAPI.addRawXP(event.getPlayer(), SkillType.WOODCUTTING.name(), event.getXpGained() * level);
                        } else if (item.getType().equals(Material.FISHING_ROD)) {
                            ExperienceAPI.addRawXP(event.getPlayer(), SkillType.FISHING.name(), event.getXpGained() * level);
                        }
                    }
                }
            }
    ...
    private void playerInteractEvent(PlayerInteractEvent event) {
        if (event.getItem() == null || event.getItem().getType().equals(Material.AIR) || !(event.getItem().hasItemMeta()) || !(event.getItem().getItemMeta().hasDisplayName()) || !(event.getItem().getItemMeta().hasLore())) {
            ...
            if (event.getItem().getItemMeta().getDisplayName().equals(api.getRPItem(RPItem.MCMMO_CREDIT_VOUCHER).getItemMeta().getDisplayName())) {
                itemtype = "credit";
            ...
            for (String string : messages.getStringList("mcmmo-voucher.redeem-" + itemtype)) {
                if (string.contains("{XP}")) {
                    string = string.replace("{XP}", formatIntUsingCommas(xp));
                }
                if (string.contains("{LEVELS}")) {
                    string = string.replace("{LEVELS}", formatIntUsingCommas(xp));
                }
                if (string.contains("{SKILL}")) {
                    string = string.replace("{SKILL}", typee.name());
                }
